...
var Messages = {
	...
	init: function() {
		...
		Connection.handle("join", function (raw) {
			Messages.list.push(Object.assign({
				kind: "join",
			}, raw))
			m.redraw()
			...
			Messages.list.map(function(msg) {
				...
				switch (msg.kind) {
					...
					case "publish":
						return m("p.message", [
							m("span.message-time",   d.toLocaleTimeString()),
							m("span.message-author", (room !== "" ? "[" + msg.room + "] " : "") + msg.author),
							m("span.message-invite", ">"),
					...
					case "join":
						return m("p.message", [
							m("span.message-time",   d.toLocaleTimeString()),
							m("span.message-author", "[" + msg.room + "] " + msg.name),
							m("span.message-info",   "joined room!")
						])
				}
